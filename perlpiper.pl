...
BEGIN {
  ...
  |  ___/ _ \ '__| |     |  ___/ | '_ \ / _ \ '__|
  | |  |  __/ |  | |____ | |   | | |_) |  __/ |
  |_|   \___|_|  |______||_|   |_| .__/ \___|_|
                                 |_|              
                                                
                ♫ TOCANDO PARA 1 MILHÃO DE SERVERS ♫
BANNER
    system('printf "\e[0m\n\a"');
}

# CORREÇÃO: Aceita argumento com nome do playbook
my $playbook_file = $ARGV[0] // 'playbook.yml';
die colored("Playbook '$playbook_file' não encontrado!\n", 'red') unless -f $playbook_file;
my $playbook  = LoadFile($playbook_file);
