...
sub run_task {
    my %p = @_;
    warn "DEBUG run_task: módulo='$p{module}'\n";
    my $ssh       = $p{ssh};
    my $ip        = $p{ip};
    my $mod       = $p{module};
    my $args      = $p{args} // {};
    ...
    if ($mod eq 'cron') {
        warn "DEBUG cron: INICIANDO módulo cron\n";
        
        my $name = $args->{name} // 'unnamed-cron-job';
        my $state = $args->{state} // 'present';
        my $job = $args->{job};
        my $user = $args->{user} // 'root';
        
	#warn "DEBUG cron: name='$name', state='$state', job='$job', user='$user'\n";
        warn "DEBUG cron: name='$name', state='$state', job='" . ($job // 'UNDEF') . "', user='$user'\n";	
        
        # Validação básica - job só é obrigatório para state=present
        if ($state eq 'present') {
            unless ($job) {
                warn "DEBUG cron: ERRO - job não especificado para state=present\n";
                return { changed => 0, error => "Parâmetro 'job' é obrigatório para cron quando state=present" };
            }
        }
        
        warn "DEBUG cron: Validação passou\n";
        
        # Constrói a linha do cron (só para state=present)
        ...
        my $cron_line = "$minute $hour $day $month $weekday " . ($job // '');
        my $cron_entry = "# $name\n$cron_line";
	...
	#my $cron_entry = "# $name\n$cron_line";
        ...
        if ($state eq 'present') {
            ...
            my $current_crontab = $ssh->capture("crontab -u $user -l 2>/dev/null || echo ''");
            chomp $current_crontab;
            
            # Verifica se o job já existe (busca pelo nome)
            if ($current_crontab =~ /#\s*\Q$name\E\s*\n/) {
                return { changed => 0 };  # Já existe, nada a fazer
            }
            
            # Remove job anterior com mesmo nome (se existir)
            my $new_crontab = $current_crontab;
            $new_crontab =~ s/#\s*\Q$name\E\s*\n[^\n]*\n?//g;
            
            # Adiciona o novo job
            $new_crontab .= "\n$cron_entry\n";
            $new_crontab =~ s/^\n+//;  # Remove linhas vazias no início
	    #$new_crontab =~ s/\n+$/\n/; # Garante apenas uma linha vazia no final
	    $new_crontab =~ s/\s*$/\n/; # Garante EXATAMENTE uma nova linha no final
            
            # Salva o novo crontab
            my $tmp_file = "/tmp/crontab-$$";
            open my $fh, '>', $tmp_file or die "Não pude criar $tmp_file: $!";
            print $fh $new_crontab;
            close $fh;
            
            $ssh->scp_put($tmp_file, $tmp_file);
            $ssh->system("crontab -u $user $tmp_file");
            $ssh->system("rm -f $tmp_file");
            unlink $tmp_file;
        ...
        if ($state eq 'absent') {
            ...
            warn "DEBUG cron/absent: Procurando job: '$name'\n";
            warn "DEBUG cron/absent: Crontab tem " . length($current_crontab) . " caracteres\n";
            
            # Verifica se o job existe (busca pelo nome)
            unless ($current_crontab =~ /#\s*\Q$name\E\s*\n/) {
                warn "DEBUG cron/absent: Job NÃO encontrado\n";
                return { changed => 0 };  # Não existe, nada a fazer
            }
            
            warn "DEBUG cron/absent: Job ENCONTRADO, removendo...\n";
	    ...
	    # Remove o job - MÉTODO CORRIGIDO
            my $new_crontab = $current_crontab;
            $new_crontab =~ s/#\s*\Q$name\E\s*\n[^\n]*\n?//g;
            $new_crontab =~ s/^\s+//;  # Remove espaços no início
            $new_crontab =~ s/\s*$/\n/; # Garante EXATAMENTE uma nova linha no final

            
            # Se ficou vazio, remove o crontab completamente
            if ($new_crontab =~ /^\s*$/) {
                $ssh->system("crontab -u $user -r 2>/dev/null || true");
            } else {
                # Salva o novo crontab
                my $tmp_file = "/tmp/crontab-$$";
                open my $fh, '>', $tmp_file or die "Não pude criar $tmp_file: $!";
        ...
        return { changed => 0, error => "Estado '$state' não suportado para cron" };
    }
